...
export default class WorkGroupWrapper {
  ...
  addBackgroundObjects() {
    ...
    bgRenderer.setUniformGetter(
      this.createUniformGetter(0, GeneralConfig.WGWrapperBgDepth, true)
    );
  }
  ...
  setupSelectionRenderer() {
    ...
    selectionObjectRenderer.setUniformGetter(
  ...
  addTimeAxisObjects() {
    ...
    timeLabels.forEach(timeLabel => {
      ...
      vertLineForTime2D.setColor(GeneralConfig.MainThemeColor_beta);
  ...
  addYAxisObjects() {
    ...
    yAxisRenderer.setUniformGetter(
      this.createUniformGetter(0, GeneralConfig.WGWrapperYAxisDepth, true)
    );
  }
  ...
  addNetworkElement(yPos, yStepSize, networkElementWrapper) {
    ...
    lineGrid2D.setColor(GeneralConfig.MainThemeColor_beta);
  ...
  findPossession = (startTime, endTime, networkElementWrapper) => {
    if (networkElementWrapper.Possessions) {
      ...
      if (selectedPossessions.length > 0)
        return Utils.clone(selectedPossessions[0]);
    }
    return null;
  };
  ...
  findNetworkElementWrapper(y) {
    const selectedNetworkElements = this.networkElementWrappers.filter(
      networkEleWrapper =>
  ...
  getSelectionObjectAt(x, y, showSelection = false) {
    const { appTimeTransform } = this;
    let selectionObject = null;
    ...
    if (xTime) {
      ...
      if (selectedNetworkElement) {
        ...
        // TODO: move this to PossessionWrapper
        const selectedPossession = this.findPossession(
          StartTime,
          EndTime,
          selectedNetworkElement
        );
  ...
  handleHover(x, y) {
    ...
    if (selectionObject.selectedPossession) {
      ...
      let xPos = this.appTimeTransform.timeToScreenX(
        selectedPossession.StartDate
      );
      let xRight = this.appTimeTransform.timeToScreenX(
        selectedPossession.EndDate
      );
      ...
      const allowedKeys = ["Name", "WorkDate", "WorkType", "WorkGroup"];
      const hoverText = selectedPossession.hoverTextGenerator(
        selectedPossession.data,
        allowedKeys
      );
  ...
  handleHoverYAxis = (x, y) => {
    ...
    let hoverText = [];
    let displayData = hoverTextGenerator(wrapperObj,["FromKmMarker","FromTrackName","Name","ToKmMarker","ToTrackName"]);
