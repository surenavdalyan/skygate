...
export default class GanttChartView extends Base {
  ...
  createTimeAxis() {
    ...
    let TimeHeaderClass = TimelineHeader;
    if (defaultRenderType === "operations") TimeHeaderClass = OpTimelineHeader;
  ...
  generateTimelabels(timeStep = 24) {
    ...
    this.appTimeTransform.setSpanInX(
      GeneralConfig.CellWidth * this.timeLabels.length
    );
  }
  ...
  createObjects() {
    ...
    this.appTimeTransform = new TimeTransform(
      Utils.getStartOfTheDay(this.inputData.startDate),
      Utils.getEndOfTheDay(this.inputData.endDate),
      () => this.render()
    );
  ...
  handleWheel(x, y, delta) {
    ...
    if (workGroupWrapperList.length > 0) {
      ...
      if (delta > 0) {
        scrolledY = this.yScrollOffset - 10;
  ...
  workGroupSelectionUpdated({ id, selectionFlag }) {
    const wgFiltered = this.workGroupWrapperList.filter(
      workGroupWrapper => workGroupWrapper.data.Id.toString() === id
    );
